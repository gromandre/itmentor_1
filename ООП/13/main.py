...
class ShapeLogger(Shapes):
    def __init__(self, log_file: str = 'shapes_log.txt'):
        super().__init__()
        self.log_file = log_file

    def __enter__(self):
        self._file = open(self.log_file, 'w', encoding='utf-8')
        self._file.write('Добавленные фигуры:\n')
        return self

    def add_shape(self, shape: BasicShape):
        super().add_shape(shape)
        # логируем каждую добавленную фигуру
        self._file.write(f'Фигура с точками: {shape.points}, площадь: {shape.square}\n')

    def __exit__(self, exc_type, exc_val, exc_tb):
        self._file.write(f'\nСуммарная площадь всех фигур: {self.square}\n')
        self._file.write(f'Количество фигур: {len(self._shapes)}\n')
        self._file.close()
        print(f'Лог сохранён в {self.log_file}')

        return False

with ShapeLogger('shapes_log.txt') as sh:
    sq1 = BasicShape([(0,0), (0,8), (4,8), (10,0)])
    sq2 = BasicShape([(0,0), (0,8), (4,8), (2,0)])

    sh.add_shape(sq1)
    sh.add_shape(sq2)

    sh.add(sq1, (9, 9))  # добавляем точку в первую фигуру
